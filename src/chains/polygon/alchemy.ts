...
const block_time_miliseconds = 2300
...
export async function getTransaction(txid: string): Promise<EthGetTransactionResult> {
    ...
    const url = `https://polygon-mainnet.g.alchemy.com/v2/${process.env.alchemy_polygon_api_key}`
    ...
    try {
        ...
        if (data.error) {
            ...
            console.error('polygon.alchemy.getTransaction.error', data.error)
        ...
        console.error('polygon.alchemy.getTransaction.error', error.response.data.error)
